...
namespace Robot.Micro.Core.Timing
{
    public class ObservableTimer : Observable, ITimer
    {
        ...
        public virtual void Start()
        {
            ...
            {
                if (_debug) Debug.Print("Cannot start, Timer already running.");
                return;
            }

            if (State == TimerState.Stopping)
            {
                if (_debug) Debug.Print("Cannot start, Timer stopping.");
                return;
            }

            State = TimerState.Running;
            if (_debug) Debug.Print("Timer started.");
            ...
            while (State == TimerState.Running)
            {
                ...
                else if (_debug) Debug.Print("Tick over ran by :" + ((currentElapsed - TickDelta).Milliseconds));
                if (_debug) Debug.Print("Total tick time: " + (LastTickTime.CurrentElapsed().Milliseconds));
                LastTickTime.Update(TickDelta);
            }

            State = TimerState.Stopped;
            if (_debug) Debug.Print("Timer stopped.");
        }
        ...
        public virtual void Stop()
        {
            if (State == TimerState.Running)
            {
                ...
                if (_debug) Debug.Print("Timer stopping.");
            }
            else if (_debug) Debug.Print("Cannot stop, Timer already stopped.");
        }
